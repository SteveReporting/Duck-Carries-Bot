        ...
        try {
            if (interaction.isChatInputCommand()) {
                ...
                if (!command) {
                    ...
                    return interaction.reply({
                        content: "❌ That command is registered in Discord but is not loaded by the bot.",
                        ephemeral: true,
                    });
                }
            ...
            if (interaction.isButton()) {
                if (interaction.customId === "create_carry") {
                    return await showCarryModal(interaction);
                }

                if (interaction.customId.startsWith("claim_")) {
                    return await claimCarry(
                        interaction,
                        client,
            ...
            if (interaction.isModalSubmit() && interaction.customId === "carry_modal") {
                return await submitCarry(interaction);
            }
        } catch (error) {
            console.error("[INTERACTION ERROR]", error);

            const message = "❌ Something went wrong while running that interaction. Check the Lunafy console for the exact error.";
