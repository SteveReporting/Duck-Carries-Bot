...
const { DUNGEONS, parseRuns } = require("../platform/dungeons");
const {
  maybeSendAbuseAlert,
...
const START_ID = "carry_request_start_v2";
const DUNGEON_SELECT_ID = "carry_request_dungeon_v2";
const DIFFICULTY_SELECT_PREFIX = "carry_request_difficulty_v2:";
const MODAL_PREFIX = "carry_request_modal_v2:";
const MAX_ACTIVE_REQUESTS = 2;

const MAIN_DUNGEONS = DUNGEONS.filter((dungeon) => dungeon.name !== "Boss Raids");
...
function dungeonSelectRow() {
  const select = new StringSelectMenuBuilder()
    ...
    .setMaxValues(1)
    .addOptions(
      MAIN_DUNGEONS.map((dungeon, index) => ({
        label: dungeon.name,
        value: String(index),
        description: `${index + 1}. Dungeon Quest progression`,
      })),
    );
...
function difficultySelectRow(dungeonIndex) {
  const dungeon = MAIN_DUNGEONS[dungeonIndex];
  const select = new StringSelectMenuBuilder()
    ...
    .setMaxValues(1)
    .addOptions(
      difficultiesForDungeon(dungeon.name).map((difficulty) => ({
        label: difficulty,
        value: DIFFICULTY_CODES[difficulty],
        description: `${dungeon.name} • ${difficulty}`,
      })),
    );
...
async function startRequest(interaction) {
  ...
  return interaction.reply({
    content: [
      ...
      "",
      "**Step 1 of 3:** Choose the dungeon you want carried.",
    ].join("\n"),
...
async function chooseDungeon(interaction) {
  ...
  const dungeon = MAIN_DUNGEONS[dungeonIndex];
  if (!dungeon) {
    return interaction.update({ content: "❌ That dungeon selection is no longer valid. Press Request a Carry again.", components: [] });
  }

  const difficulties = difficultiesForDungeon(dungeon.name);
  return interaction.update({
    content: [
      "## 🍺 Request a Carry",
      `🏰 **Dungeon:** ${dungeon.name}`,
      "",
      "**Step 2 of 3:** Choose the difficulty.",
      EARLY_DUNGEONS.has(dungeon.name)
...
async function chooseDifficulty(interaction) {
  const dungeonIndex = Number.parseInt(interaction.customId.slice(DIFFICULTY_SELECT_PREFIX.length), 10);
  const dungeon = MAIN_DUNGEONS[dungeonIndex];
  const difficultyCode = interaction.values[0];
  const difficulty = CODE_TO_DIFFICULTY[difficultyCode];

  if (!dungeon || !difficulty || !difficultiesForDungeon(dungeon.name).includes(difficulty)) {
    return interaction.update({ content: "❌ That selection is no longer valid. Press Request a Carry again.", components: [] });
  }

  const modal = new ModalBuilder()
    .setCustomId(`${MODAL_PREFIX}${dungeonIndex}:${difficultyCode}`)
    .setTitle(`${dungeon.name} • ${difficulty}`.slice(0, 45));
...
async function submitRequest(interaction) {
  ...
  const encoded = interaction.customId.slice(MODAL_PREFIX.length).split(":");
  const dungeonIndex = Number.parseInt(encoded[0], 10);
  const difficulty = CODE_TO_DIFFICULTY[encoded[1]];
  const dungeon = MAIN_DUNGEONS[dungeonIndex];

  if (!dungeon || !difficulty || !difficultiesForDungeon(dungeon.name).includes(difficulty)) {
    return interaction.reply({ content: "❌ The selected dungeon or difficulty is invalid. Please start the request again.", ephemeral: true });
  }
...
module.exports = {
  ...
  async execute(interaction) {
    try {
      ...
      if (interaction.isStringSelectMenu() && interaction.customId.startsWith(DIFFICULTY_SELECT_PREFIX)) {
      ...
      console.error("[CARRY REQUEST PANEL V2]", error);
